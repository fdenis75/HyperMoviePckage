...
public final class LayoutProcessor {
    ...
    public func calculateLayout(
        ...
        logger.debug("🎯 Starting layout calculation - AR: \(originalAspectRatio), Count: \(thumbnailCount), Width: \(mosaicWidth)")
        logger.debug("⚙️ Layout mode - Auto: \(useAutoLayout), Custom: \(useCustomLayout), Density: \(density.name)")
    ...
    private func calculateCustomLayout(
        ...
        logger.debug("🎨 Calculating custom layout - Density: \(density)")
        ...
        logger.debug("📐 Initial grid - Large: \(largeRows)x\(largeCols), Small: \(smallRows)x\(smallCols)")
        ...
        logger.debug("✅ Custom layout complete - Final grid: \(totalRows)x\(smallCols)")
        return layout
    }
